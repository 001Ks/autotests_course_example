...
class PersonInfo:
    def __init__(self, fullname: str, age: int, *department: str):
    ...
    def new_salary(self):
        letters_dict = {}
        for letter in ''.join(self.department).replace(' ', ''):
            if letter in letters_dict:
                letters_dict[letter] += 1
            else:
                letters_dict[letter] = 1

        top_letters = sorted(letters_dict.keys(), key=lambda letter: letters_dict[letter], reverse=True)

        sum_points = 0
        for letter in top_letters[:3]:
            sum_points += letters_dict[letter]

        return 1337 * self.age * sum_points
