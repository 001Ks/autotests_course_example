...
def everything_for_your_cat(cats_data):
    ...
    # Что делаем: Группируем котов по владельцам и форматируем данные котов и их владельцев
    owner_cats = {}
    for cat_name, cat_age, owner_name, owner_surname in cats_data:
        owner = f'{owner_name} {owner_surname}'
        cat = f'{cat_name}, {cat_age}'
        if owner in owner_cats:
            owner_cats[owner] += '; ' + cat
        else:
            owner_cats[owner] = cat

    # Что делаем: Формируем итоговый результат по требованиям задачи
    our_str = ''
    for owner, cats in owner_cats.items():
        our_str = our_str + owner + ': ' + cats + '\n'
