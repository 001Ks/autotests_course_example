...
def josephus_task(num_people, kill_num):
    ...
    people_list = []
    for human in range(num_people):
        people_list.append(human + 1)

    count = 1  # Счетчик текущего элемента
    idx = 0  # Индекс текущего элемента
    while len(people_list) > 1:
        if count % kill_num == 0:
            people_list.pop(idx)  # Удаляем каждый N-кратный элемент
        else:
            # Увеличиваем индекс только если элемент не был удален
            idx += 1

        # Если индекс достиг длины - значит нужно начать круг заново. Обнуляем индекс
        if idx == len(people_list):
            idx = 0

        # Каждую итерацию увеличиваем счетчик, даже если элемент выбыл
        count += 1
